...
# Current predictions (Lag28, TopK, PCA)
...
# ---------------- PLOT ----------------
st.subheader(f"📊 Prediction vs Actual ({model_option}) — {station_option}")

fig_pred = px.line(df_pred, x="hour", y=["actual_rides", "predicted_rides"], labels={"value": "rides", "hour": "Timestamp"}, title="Past Predictions")
st.plotly_chart(fig_pred, use_container_width=True)

st.subheader(f"🔮 Future Forecast ({model_option}) — {station_option}")

fig_fut = px.line(df_fut, x="hour", y="predicted_rides", labels={"predicted_rides": "rides", "hour": "Future Time"}, title="Future Forecast")
