...
def upload_lag28_predictions_and_models(project):
    fs = project.get_feature_store()
    registry = project.get_model_registry()

    # Load MAE summary
    mae_df = pd.read_csv(os.path.join(RESULTS_DIR, "lgbm_lag28_mae_summary.csv"))

    for station_id in TOP_STATIONS:
        # ---------------------------
        # 1. Upload Predictions CSV
        # ---------------------------
        pred_path = os.path.join(RESULTS_DIR, f"predictions_lgbm_lag28_{station_id}.csv")
        if not os.path.exists(pred_path):
            print(f"❌ Missing predictions: {pred_path}")
            continue

        df = pd.read_csv(pred_path, parse_dates=["hour"])
        fg_name = f"{PREDICTION_STORE_NAME}_lag28_{station_id.lower()}"
        ...
        fg.insert(df)
        print(f"✅ Uploaded lag28 predictions → {fg_name}")

        # ---------------------------
        # 2. Register Model (.pkl)
        # ---------------------------
        model_path = os.path.join(MODELS_DIR, f"lgbm_lag28_model_{station_id}.pkl")
        if not os.path.exists(model_path):
            print(f"❌ Missing model: {model_path}")
            continue

        model = joblib.load(model_path)
        tmp_dir = f"tmp_model_lag28_{station_id}"
        ...
        model_obj = registry.python.create_model(
            name=f"citibike_lag28_{station_id}",
            metrics={"mae": mae},
            model_schema=ModelSchema(Schema(X_dummy), Schema(y_dummy)),
            description=f"Lag-28 model for {station_id}"
        )
        model_obj.save(tmp_dir)
        print(f"✅ Registered model: citibike_lag28_{station_id}")
...
def main():
    ...
    upload_lag28_predictions_and_models(project)
